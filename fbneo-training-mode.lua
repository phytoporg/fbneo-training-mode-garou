...
local defaultconfig = {
    ...
    hud = {
        ...
        p1guarddamagemetertopy = 32,
        p1guarddamagemeterleftx = 20,
        p2guarddamageenabled = false,
        p2guarddamagetextx = 296,
        p2guarddamagetexty = 32,
        p2guarddamagemetertopy = 32,
        p2guarddamagemeterrightx = 8,
        guarddamagemeterheight = 32,
        guarddamagemeterwidth = 32,
        guarddamagetextcolor = 0xFFFF00FF,
        guarddamagetextcolordanger = 0xFF0000FF,
        guarddamagemetercoloroutline = 0xFFFF00FF,
        guarddamagemetercolorbg = 0xFFFF0030,
        guarddamagemetercolorfill = 0xFFFF00FF,
        guarddamagemetercolordangeroutline = 0xFF0000FF,
        guarddamagemetercolordangerbg = 0xFF000030,
        guarddamagemetercolordangerfill = 0xFF0000FF,
    },
...
hud = {
    ...
    p1guarddamagemetertopy = config.hud.p1guarddamagemetertopy,
    p1guarddamagemeterleftx = config.hud.p1guarddamagemeterleftx,
    ...
    p2guarddamagemetertopy = config.hud.p2guarddamagemetertopy,
    p2guarddamagemeterrightx = config.hud.p2guarddamagemeterrightx,
    guarddamagemeterheight = config.hud.guarddamagemeterheight,
    guarddamagemeterwidth = config.hud.guarddamagemeterwidth,
    guarddamagetextcolor = config.hud.guarddamagetextcolor,
    guarddamagetextcolordanger = config.hud.guarddamagetextcolordanger,
    guarddamagemetercoloroutline = config.hud.guarddamagemetercoloroutline,
    guarddamagemetercolorbg = config.hud.guarddamagemetercolorbg,
    guarddamagemetercolorfill = config.hud.guarddamagemetercolorfill,
    guarddamagemetercolordangeroutline = config.hud.guarddamagemetercolordangeroutline,
    guarddamagemetercolordangerbg = config.hud.guarddamagemetercolordangerbg,
    guarddamagemetercolordangerfill = config.hud.guarddamagemetercolordangerfill,
}
...
local drawP1GuardDamageMeter = function(damage, dangerDamage, maxDamage)
    if damage == nil then damage = 0 end
    if dangerDamage == nil then dangerDamage = 0 end
    if maxDamage == nil then maxDamage = 0 end

    -- TODO: clean up to mirror structure of drawP2GuardDamageMeter
    local x, y, w, h
    x = hud.p1guarddamagemeterleftx
    y = hud.p1guarddamagemetertopy
    h = hud.guarddamagemeterheight
    w = maxDamage

    -- TODO: minimize overdraw

    -- Outer box
    -- "Normal" range outline
    gui.drawline(x, y, x + dangerDamage, y, hud.guarddamagemetercoloroutline)
    gui.drawline(x, y, x, y + h, hud.guarddamagemetercoloroutline)
    gui.drawline(x, y + h, x + dangerDamage, y + h, hud.guarddamagemetercoloroutline)
    -- "Danger" range outline
    gui.drawline(x + dangerDamage, y, x + w, y, hud.guarddamagemetercolordangeroutline)
    gui.drawline(x + w, y, x + w, y + h, hud.guarddamagemetercolordangeroutline)
    gui.drawline(x + dangerDamage, y + h, x + w, y + h, hud.guarddamagemetercolordangeroutline)

    -- Inner background fill
    gui.box(x, y, x + dangerDamage, y + h, hud.guarddamagemetercolorbg, 0)
    gui.box(x + dangerDamage - 1, y, x + w, y + h, hud.guarddamagemetercolordangerbg, 0)

    -- Inner damage value fill
    gui.box(x, y, x + math.min(damage, dangerDamage), y + h, hud.guarddamagemetercolorfill, 0)
    if damage >= dangerDamage then
        gui.box(x + dangerDamage - 1, y, x + damage, y + h, hud.guarddamagemetercolordangerfill, 0)
    end
end

local drawP2GuardDamageMeter = function(damage, dangerDamage, maxDamage)
    if damage == nil then damage = 0 end
    if dangerDamage == nil then dangerDamage = 0 end
    if maxDamage == nil then maxDamage = 0 end

    local wmax, wd, y1, y2, xr, xl, xd, xfr, xfl
    wmax = maxDamage                            -- Full width of the meter
    wd = maxDamage - dangerDamage               -- Width of the "danger zone"
    y1 = hud.p2guarddamagemetertopy             -- Top vertical coordinate
    y2 = y1 + hud.guarddamagemeterheight        -- Bottom vertical coordinate
    xr = hud.p2guarddamagemeterrightx           -- Right horizontal coordinate
    xl = hud.p2guarddamagemeterrightx - wmax    -- Left horizontal coordinate
    xd = xl + wd                                -- "Danger" horizontal coordinate
    xfr = xr - math.min(damage, dangerDamage)   -- Right fill horizontal coordinate
    xfl = xfr - (damage - dangerDamage)         -- Left fill horizontal coordinate

    -- TODO: minimize overdraw

    -- Outer box
    -- "Normal" range outline
    gui.drawline(xd, y1, xr, y1, hud.guarddamagemetercoloroutline)
    gui.drawline(xr, y1, xr, y2, hud.guarddamagemetercoloroutline)
    gui.drawline(xd, y2, xr, y2, hud.guarddamagemetercoloroutline)
    -- "Danger" range outline
    gui.drawline(xl, y1, xd, y1, hud.guarddamagemetercolordangeroutline)
    gui.drawline(xl, y1, xl, y2, hud.guarddamagemetercolordangeroutline)
    gui.drawline(xl, y2, xd, y2, hud.guarddamagemetercolordangeroutline)

    -- Inner background fill
    gui.box(xd, y1, xr, y2, hud.guarddamagemetercolorbg, 0)
    gui.box(xl, y1, xd + 1, y2, hud.guarddamagemetercolordangerbg, 0)
    ...
    gui.box(xfr, y1, xr, y2, hud.guarddamagemetercolorfill, 0)
    if damage >= dangerDamage then
        gui.box(xfl, y1, xfr + 1, y2, hud.guarddamagemetercolordangerfill, 0)
    end
end
...
setRegisters = function() -- pre-calc stuff
    ...
    if availablefunctions.readplayeroneguarddamage and availablefunctions.readplayeronemaxguarddamage then
        ...
        -- P1 guard damage meter
        table.insert(HUDElements, {name = "p1guarddamagemeter", x = function(n) if n then changeConfig("hud", "p1guarddamagemetertopx", n, hud) end return hud.p1guarddamagemetertopx end, y = function(n) if n then changeConfig("hud", "p1guarddamagemetertopy", n, hud) end return hud.p1guarddamagemetertopy end, enabled = function(n) if n~=nil then changeConfig("hud", "p1guarddamageenabled", n, hud) end return hud.p1guarddamageenabled end, drawfunc = function() drawP1GuardDamageMeter(guardvars.p1.guarddamage, guardvars.p1.guarddamagedanger, guardvars.p1.maxguarddamage) end})
    ...
    if availablefunctions.readplayertwoguarddamage and availablefunctions.readplayertwomaxguarddamage then
        ...
        -- P2 guard damage meter
        table.insert(HUDElements, {name = "p2guarddamagemeter", x = function(n) if n then changeConfig("hud", "p2guarddamagemeterrightx", n, hud) end return hud.p2guarddamagemeterrightx end, y = function(n) if n then changeConfig("hud", "p2guarddamagemetertopy", n, hud) end return hud.p2guarddamagemetertopy end, enabled = function(n) if n~=nil then changeConfig("hud", "p2guarddamageenabled", n, hud) end return hud.p2guarddamageenabled end, drawfunc = function() drawP2GuardDamageMeter(guardvars.p2.guarddamage, guardvars.p2.guarddamagedanger, guardvars.p2.maxguarddamage) end})
